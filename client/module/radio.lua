...
function radioui(bool)
	if bool then
		SendNUIMessage({
			radio = true,
			radioChannel = radioChannel.. ' MHz',
		})
	else
		SendNUIMessage({
			radio = false,
...
function removePlayerFromRadio(plySource)
	print('[radio] Removing player from radio: ' .. plySource)
...
RegisterCommand('jointochanel', function(source, args, raw)
	if args[1] and tonumber(args[1]) > 0 then
		setRadioChannel(tonumber(args[1]))
	else 
